...
<body>
                    ...
                    <h3 id="expense" align="center">Aegis QR Fencing- Report</h3>
					 ...
					 $date1=date("Y-m-d");?>
       <form   method='POST' form id="form" action="<?php echo base_url();?>/index.php/Report/qrreport/"<?php echo $_POST['search'],$_POST['username']?>  >
...
//	$query="SELECT supervisor,MAX(distance) as dis,itime,idistance,gt.id,tdate,application_id,location FROM aegis_geofence as gt, vehicleno_imei_mapping as vim   where gt.imeino=vim.imeino and application_id='3LGB6U2' and isActive='Y' and tdate='$search'  GROUP BY gt.imeino";
	
  $query="SELECT supervisor,MAX(distance) as dis,itime,idistance,gt.id,tdate,application_id,location FROM aegis_geofence as gt, vehicleno_imei_mapping as vim   where gt.userid = vim.userid and application_id='$app' and isActive='Y' and tdate='$search'  GROUP BY gt.userid";
   ...
   $result=mysql_query($query);
		...
		// $sql2=mysql_query("SELECT  supervisor FROM vehicleno_imei_mapping where  application_id='3LGB6U2' and  isActive='Y' and         imeino not in (Select imeino from aegis_geofence where tdate='$search'  GROUP BY imeino )"); 
		 
		  $sql2=mysql_query("SELECT  supervisor FROM vehicleno_imei_mapping where  application_id='$app' and  isActive='Y' and         userid not in (Select userid from aegis_geofence where tdate='$search'  GROUP BY userid )"); 
...
$date=date("Y-m-d");
 ...
 $query="SELECT supervisor,MAX(distance) as dis,itime,gt.id,tdate,application_id,location FROM aegis_geofence as gt, vehicleno_imei_mapping as vim   where gt.userid=vim.userid and application_id='$app'  and isActive='Y'  and tdate='$date' GROUP BY vim.`userid`  ";
